...
export const TopSalesContext = createContext<ISearchKeyword>({});

// 만들어진 값(CommonCodeCotext)에 searchKeyword, setSearchKeyword을 넣어서 자식 노드에서
// 자유롭게 searchKeyword과 setSearchKeyword를 호출한다.
export const TopSalesProvider: FC<{
    children: React.ReactNode | React.ReactNode[];
}> = ({ children }) => {
    const [searchKeyword, setSearchKeyword] = useState<object>({});
    return (
        <TopSalesContext.Provider value={{ searchKeyword, setSearchKeyword }}>{children}</TopSalesContext.Provider>
    );
};