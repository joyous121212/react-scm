...
export const ShoppingContext = createContext<ISearchKeyword>({});

// 만들어진 값(CommonCodeContext)에 searchKeyword, setSearchKeyword을 넣어서 자식 노드에서
// 자유롭게 searchKeyword과 setSearchKeyword를 호출한다.
export const ShoppingProvider: FC<{
    children: React.ReactNode | React.ReactNode[];
}> = ({ children }) => {
    const [searchKeyword, setSearchKeyword] = useState<object>({});
    return (
        <ShoppingContext.Provider value={{ searchKeyword, setSearchKeyword }}>{children}</ShoppingContext.Provider>
    );
};
