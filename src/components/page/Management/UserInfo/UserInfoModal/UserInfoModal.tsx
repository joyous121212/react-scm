 ...
 const [detailInfo, setDetailInfo] = useState<IUserDetialInfo>({
    ...
    useEffect(  () => {
        ...
        async function x(){
            ...
            const res1:any = await  userInfoDetailApi(
                UserInfo.userInfoDetail,
                { loginID: LoginId }
            );
            const res2:any= await  userInfoDetailApi(
                UserInfo.detailsearch,
...
//박스
    ...
    const handleDeleteYes = async () => {
        setAction('삭제');
           await  axios.post("/management/userDeleteBody.do",{loginID:LoginId})
           .then((res:any)=>{
    ...
    const handleRestoreYes = async () => {
        setAction('복구');
        await  axios.post("/management/userRecoveryBody.do",{loginID:LoginId})
        .then((res:any)=>{
         if(res.data.result==="success"){
             alert("복구에 성공하였습니다.")
         }else{
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td colSpan={3} id='detail_code'>
                                   {isdetail? 
                                    ...
                                    <select name='detailCode' value={detailInfo?.detailCode}  onChange={detaileHandleChange}>
                                    {/* {isdetail ? (
                        ...
                        <tr>
                            ...
                            <td colSpan={6}>
                                {isdetail ? (
                                    <StyledInput  name="email" value={detailInfo?.email} onChange={detaileHandleChange} ></StyledInput>
                                ) : (
